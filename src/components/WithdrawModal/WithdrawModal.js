...
import ForeignerModal from "../ForeignerModal/ForeignerModal";
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-5">
          <div className="md:grid grid-cols-2">
            <div className="my-4">
              <select
                onChange={(e) => setIsIndian(e.target.value)}
                id="countries"
                class="bg-gray-50 text-black text-sm shadow-md rounded-lg block md:w-5/6 w-full p-2"
              >
                <option selected>Select</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <label for="countries" class="block text-sm text-black mt-1">
            ...
            <div className="my-4">
              <select
                id="countries"
                disabled={isIndian !== "Yes"}
                class="bg-gray-50 text-black text-sm shadow-md rounded-lg block md:w-5/6 w-full p-2 disabled:cursor-not-allowed"
                onChange={(e) => setHaveGST(e.target.value)}
              >
                <option selected>Select</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
            ...
            <div className="my-4 block text-center">
              <button className="disabled:bg-interactive-light-disabled disabled:cursor-not-allowed text-white bg-interactive-light text-button hover:bg-interactive-light-hover focus:bg-interactive-light-focus active:bg-interactive-light-active font-bold rounded-full uppercase flex gap-1 px-3 py-2">
                Save and Next
              </button>
            </div>
          </div>
        ...
        {showGSTModal ? (
          <GSTModal setShowGstModal={setShowGstModal} />
        ) : showIndianModal ? (
          <IndianModal setShowIndianModal={setShowIndianModal} />
        ) : showForeignerModal ? (
          <ForeignerModal setShowForeignerModal={setShowForeignerModal} />
        ) : (
          ""
        )}
      </div>
