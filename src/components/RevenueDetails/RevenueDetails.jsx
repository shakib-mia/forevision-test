      ...
      <div className='w-screen h-screen fixed top-0 left-0 z-[9999] flex justify-center items-center backdrop-blur-[1px]'>
        <div className='w-3/5 h-[80vh] relative overflow-x-visible rounded-2xl overflow-y-auto p-3 px-5' style={{ backgroundColor: '#ffffffd9' }}>
          ...
          <div className='ml-4 mt-2'>
            <p style={{ fontSize: "37.9px", lineheight: "24px" }} className='ml-3 mb-3 text-heading-6-bold text-grey-dark my-1'>{result[0].song_name}</p>
            <p style={{ fontSize: "37.9px", lineheight: "24px" }} className='ml-3 text-heading-6-bold text-grey-dark'>{result[0].isrc}</p>
          </div>
          {/* //  list heading  */}
          <ul className="grid grid-cols-3 gap-3 mt-5 mb-2 text-heading-6-bold text-grey-dark">
            {options.map((item, key) => <li key={key} style={{ color: "#1B96FF" }} className={`${item === 'isrc' ? 'uppercase' : 'capitalize'} text-center`}>{item === "uploadTime" ? "Month" : item.includes("_") ? item.split("_").join(" ") : item === 'final revenue' ? 'Revenue' : item === 'total' ? 'Total Stream' : item === 'platformName' ? "Platform Name" : item}</li>)}
          </ul>
