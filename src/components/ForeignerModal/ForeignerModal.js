...
const ForeignerModal = ({ setShowForeignerModal }) => {
  ...
  const [secondaryUID, setSecondaryUID] = useState({});
  ...
  const handleSubmit = (e) => {
    ...
    const govtData = new FormData();
    govtData.append("file", govtId);

    const secondaryData = new FormData();
    secondaryData.append("file", secondaryUID);

    const cancelledData = new FormData();
    cancelledData.append("file", cancelledCheck);
    ...
    if (accountNumber.value === accountNumberCheck.value) {
      ...
      Promise.all(uploadPromises).then((res) => {
        // console.log(res);
        const [govt, secondary, cancelled] = res;
        const govtUrl = govt.data.url;
        const secondaryUrl = secondary.data.url;
        const cancelledUrl = cancelled.data.url;
      ...
      toast.error("Account Number didn't match");
    }
  ...
  return (
    <div className="w-screen h-screen fixed top-0 left-0 z-[9999] flex justify-center items-center backdrop-blur-[1px]">
      ...
      >
        <button
          onClick={() => setShowForeignerModal(false)}
        ...
        <form onSubmit={(e) => handleSubmit(e)}>
          <div className="md:grid grid-cols-2">
              ...
              <div className="bg-gray-50 text-black text-sm shadow-md rounded-lg block md:w-5/6 w-full p-2">
                ...
                >
                  <p className="overflow-hidden mr-2 h-3">
                    {!govtId.name ? "Upload Government ID" : govtId.name}
                  </p>
                  <svg
                    width="14"
                    height="14"
            ...
            <div className="my-4">
              <div className="bg-gray-50 text-black text-sm shadow-md rounded-lg block md:w-5/6 w-full p-2">
                <label
                  htmlFor="secondaryUID"
                  className="btn flex justify-between items-center"
                >
                  <p className="overflow-hidden mr-2 h-3">
                    {!secondaryUID.name
                      ? "Upload Secondary User ID"
                      : secondaryUID.name}
                  </p>
                  <svg
                    width="14"
                ...
                <input
                  ...
                  name="secondaryUID"
                  id="secondaryUID"
                  onChange={(e) => setSecondaryUID(e.target.files[0])}
                  style={{ display: "none" }}
                />
              </div>
              <label
                ...
                className="block text-sm text-black mt-1"
              >
                Upload your Secondary Unique ID
              </label>
            </div>
            <div className="my-4">
              <div className="bg-gray-50 text-black text-sm shadow-md rounded-lg block md:w-5/6 w-full p-2">
                <label
                  htmlFor="cancelledCheck"
                  className="btn flex justify-between items-center"
                >
                  <p className="overflow-hidden mr-2 h-3">
                    {!cancelledCheck.name
                      ? "Upload Cancelled Check File"
                      : cancelledCheck.name}
                  </p>
                  <svg
                    width="14"
                ...
                <input
                  type="file"
                  name="cancelledCheck"
                  onChange={(e) => setCancelledCheck(e.target.files[0])}
                  id="cancelledCheck"
                  style={{ display: "none" }}
                />
              </div>
