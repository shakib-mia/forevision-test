                    ...
                    <div className="relative h-full flex items-center">
                      <label className="absolute top-0 bottom-0 w-fit h-fit left-2 m-auto">
                        <div className="flex items-center gap-1">
                          <span>+{selectedCode}</span> <img src={downArrow} alt="" className={`!h-1 !w-auto transition ${showCodes && 'rotate-180'}`} />
                        </div>
                        <input type="checkbox" className="hidden" onChange={e => setShowCodes(e.target.checked)} />
                        {showCodes && <div className="absolute top-4 h-7 overflow-y-auto w-7 bg-white p-2 shadow-xl">
                          {codes.map((code) => <CountryCodeItem setSelectedCode={setSelectedCode} countryCodes={code} />)}
                        </div>}
                      </label>
